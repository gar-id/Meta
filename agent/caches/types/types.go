...
type MetaHandlerAgent struct {
	AgentConfig struct {
		Server struct {
			...
			EncryptedAgentToken    string `yaml:"encryptedAgentToken" json:"encrypted_agent_token"` // will be use for encrypted meta handler API agent token
		...
		Log struct {
			...
			Level    string `yaml:"logLevel" json:"log_level"`       // meta handler log level
		} `yaml:"log" json:"log"`
